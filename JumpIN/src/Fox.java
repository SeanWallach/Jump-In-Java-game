...
public class Fox extends MovableGamePiece {
	...
	public Fox(String s, int xpos, int ypos, boolean direction) {
		// The x and y positions that will be stored will be the smaller values (closer
		// to (0, 0))
	...
	/**
	 * Method to get the x-coordinate of the back of the Fox (since the fox takes up
	 * 2 tiles)
	 * 
	...
	/**
	 * Method to get the y-coordinate of the back of the Fox (since the fox takes up
	 * 2 tiles)
	 * 
	...
	/**
	 * This method checks that the passed in X and Y values are a legal move for the
	 * fox. Rules of a fox: cannot jump and can only move vertically or horizontally
	 * depending on the setup of the fox.
	 * 
	...
	private boolean canMove(int newX, int newY) {
		...
		if (upDown && (newX != this.x))
			return false;
		else if (!upDown && (newY != this.y))
			return false;
		...
		if (upDown) {
			if (newY < this.y - 1)
				return false;
			else if (newY > this.backY + 1)
				return false;
		} else {
			if (newX < this.x - 1)
				return false;
			else if (newX > this.backX + 1)
				return false;
		}
		return true;
	}
	...
	/**
	 * This extends the GamePiece placeOnTiles method because a fox needs to place
	 * both the front and back onto the tiles.
	 */
	@Override
